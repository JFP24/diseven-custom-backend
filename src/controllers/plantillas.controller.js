...
export const guardarPlantilla = asyncHandler(async (req, res) => {
  ...
  if (yaExiste) {
    return res.status(409).json({
      message: "Ya existe una plantilla con ese lugar en este proyecto",
      data: yaExiste,
    });
  }
...
export const listarPlantillasPorProyecto = asyncHandler(async (req, res) => {
...
export const obtenerPlantillaPorId = asyncHandler(async (req, res) => {
  const { id } = req.params;
...
export const actualizarPlantilla = asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(400).json({ message: "ID inválido" });
  }
